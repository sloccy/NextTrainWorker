...
describe("patchLiveWith", () => {
  ...
  it("per-stop delay override wins over trip-level on-time", () => {
    ...
    const out = new Uint8Array(template.length);
    // Trip is on-time at the trip level, but 3 min late at this stop
    ...
    expect(result[statusOff]).toBe(3); // per-stop wins
  });
  ...
  it("per-stop early: -4 min → status byte 0xFC (= -4 as s8)", () => {
    ...
    const result = patchLiveWith(
      out, template, offsets, stopOffsets,
      new Map([[tripHash, 0]]),
      new Map([[stopHash, (-4) & 0xff]]), // early 4 min
    );
    expect(result[offsets.get(tripHash)![0]]).toBe(0xFC); // -4 as u8
  });
  ...
  it("stop override without trip-level entry — stop still patches if trip-level already in template", () => {
    ...
    const out = new Uint8Array(template.length);
    // No tripStatus entry — but stop override still writes
    const result = patchLiveWith(
      out, template, offsets, stopOffsets,
      new Map(),
      new Map([[stopHash, 5]]),
